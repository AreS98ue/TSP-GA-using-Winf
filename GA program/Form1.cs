...
namespace TSP_GA_WINF
{
    public partial class Form1 : Form
    {
        ...
        Pocetak:
            ...
            double[] Nizsort = new double[velicinaPop * 2];
            Array.Copy(ukupnevred, Nizsort, velicinaPop * 2);
            Array.Sort(Nizsort);
            int[,] ModGen = new int[velicinaPop, DuzinaHrom];
            
            for (int i = 0; i < velicinaPop; i++)
            {
                double VredNajmanjih = Nizsort[i];
                int pozicija = Array.IndexOf(ukupnevred, VredNajmanjih);
